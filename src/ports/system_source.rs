...
use crate::domain::{CpuMetrics, Disk, LoadAverage, MemoryMetrics, NetworkInterface, Temperature};
...
pub trait SystemSource: Send + Sync {
    ...
    /// Get temperature readings from available sensors.
    /// Returns empty vec if no sensors are available (graceful degradation).
